...
由於 Jupyter Notebook 內部已在運行 `asyncio` 事件迴圈，如果直接使用同步 API (`sync_playwright()`) 會導致衝突並拋出錯誤（例如：`Error: It looks like you are using Playwright Sync API inside the asyncio loop.`）。

因此，在 Jupyter Notebook 中必須使用**非同步 API (`async_playwright`)**。且因為 Jupyter Notebook 支援 Top-level await，您可以直接執行以下程式碼：
...
[專案 01：網站健康檢查 →](../../實戰專案/專案01_網站健康檢查/README.md)
